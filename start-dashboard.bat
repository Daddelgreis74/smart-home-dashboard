...
:: Phase 1: Node.js & npm Prüfung
echo %BLUE%%BOLD%=== Phase 1: System-Prüfung ===%NC%
...
echo.
echo %PROMPT% Möchtest du Node.js automatisch über den Windows Package Manager (winget) installieren?
...
echo %INFO% Aktualisiere System-Pfade für diese Sitzung...

:: Pfade neu laden (für die aktuelle Session)
...
:no_node
echo.
echo %ERROR% Node.js ist zwingend erforderlich, um das Dashboard auszuführen!
...
:node_ok
echo   ➜  Node.js:  %GREEN%Installiert [✔]%NC%

:: 2. Prüfen, ob npm erreichbar ist
where npm >nul 2>nul
if %errorlevel% neq 0 (
    echo.
...
echo   ➜  npm:      %GREEN%Installiert [✔]%NC%
echo.

:: Phase 2: Abhängigkeiten prüfen (npm install)
echo %BLUE%%BOLD%=== Phase 2: Abhängigkeiten ===%NC%
if exist node_modules goto start_server

echo %INFO% "node_modules" wurde nicht gefunden.
echo %INFO% Installiere benötigte JavaScript-Bibliotheken (npm install)...
echo.
call npm install
if %errorlevel% neq 0 (
    echo.
...
echo %INFO% Öffne Browser: %CYAN%https://localhost:8443%NC%
echo.

:: Startet den Webbrowser verzögert (um dem Server Zeit zum Booten zu geben)
